...
export class SacramentoRPGActor extends Actor {
  ...
  _prepareCharacterData(actorData) {
    ...
    if (attributes.actions) {
        attributes.actions.value + corVal;
    }
  ...
  async _preUpdate(changed, options, user) {
    ...
    // Verifica se o valor de dor está sendo alterado
    ...
    if (newPain !== undefined) {
        ...
        if (newPain <= 0) {
            ...
            const currentLife = actorData.health.value;
            const maxPain = actorData.pain.max || 6;

            // 1. Reduz 1 ponto de Vida (se ainda tiver)
            if (currentLife > 0) {
                foundry.utils.setProperty(changed, "system.health.value", currentLife - 1);
            }

            // 2. Reseta a Dor para o máximo
            foundry.utils.setProperty(changed, "system.pain.value", maxPain);
        }
    }
  }
